...
async def test_deposit(client: AsyncClient):
        response = await client.post("/api/v1/wallets/test/operation", params={"amount": 100, "operationType": "deposit"})
...
async def test_deposit2(client: AsyncClient):
        response = await client.post("/api/v1/wallets/test2/operation", params={"amount": 100, "operationType": "deposit"})
        assert response.status_code == 200
...
async def test_deposit_wallet_not_found(client: AsyncClient):
        response = await client.post("/api/v1/wallets/test3/operation", params={"amount": 100, "operationType": "deposit"})
        assert response.status_code == 404
        assert response.json() == {"error": "Wallet not found"}

@pytest.mark.anyio
async def test_withdraw(client: AsyncClient):
        response = await client.post("/api/v1/wallets/test/operation", params={"amount": 200, "operationType": "withdraw"})
        assert response.status_code == 200
        assert response.json() == {"message": "Successful.", "balance": 9900, "wallet_uuid": "test"}

@pytest.mark.anyio
async def test_withdraw2(client: AsyncClient):
        response = await client.post("/api/v1/wallets/test2/operation", params={"amount": 200, "operationType": "withdraw"})
        assert response.status_code == 200
...
async def test_withdraw_insufficient(client: AsyncClient):
        response = await client.post("/api/v1/wallets/test2/operation", params={"amount": 10000, "operationType": "withdraw"})
        assert response.status_code == 422
