    ...
    async with async_session() as session:
        balance = await session.execute(
            select(Wallet.balance).where(
                Wallet.wallet_uuid==WALLET_UUID))
        balance = balance.scalars().first()
        if balance is None:
            return JSONResponse({"error": "Wallet not found"}, status_code=404)
        return JSONResponse({"message": "Successful.", "balance": balance, "wallet_uuid": WALLET_UUID})
...
async def operation(WALLET_UUID: str, amount: int, operationType: str):
    async with async_session() as session:
        ...
        return {"message": "Successful.", "balance": wallet.balance, "wallet_uuid": wallet.wallet_uuid}
